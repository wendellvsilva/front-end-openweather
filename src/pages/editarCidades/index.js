...
export function EditarCidadePage() {
    const { id } = useParams();
    ...
    useEffect(() => {
        if (id) {
            axios.get(`http://localhost:8080/cidades/${id}`)
                .then(response => {
                    const cidade = response.data;
                    if (cidade && cidade.dadosCadastroClima) {
                        setCidade(cidade.cidade);
                        setData(moment(cidade.dadosCadastroClima.data, 'DD/MM/YYYY'));
                        setUmidade(cidade.dadosCadastroClima.umidade);
                        setPrecipitacao(cidade.dadosCadastroClima.precipitacao);
                        setTemperaturaMaxima(cidade.dadosCadastroClima.tempMaxima);
                        setTemperaturaMinima(cidade.dadosCadastroClima.tempMinima);
                        setClima(cidade.dadosCadastroClima.situacaodadosCadastroClima);
                        setVento(cidade.dadosCadastroClima.velVento);
                        setTurno(cidade.dadosCadastroClima.turno);
                    }
                })
                .catch(error => {
                    console.error("Houve um erro ao buscar os dados da cidade:", error);
                });
        }
    }, [id]);
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            const resposta = await axios.put(`http://localhost:8080/cidades/${id}`, dados, {
                headers: {
    ...
    return (
        <main className={style.classe_cidade}>
            ...
            />
            <h1>Editar dados meteorológicos</h1>
